...
homeSite.controller('CreateAccountTeacherController', function ($scope, $http) {
    $scope.submit = function (formTeacher) {

        $scope.teacher.urlImageProfile = $('#inputImage').val();

        $scope.submitButtonCreate = true;
        if (formTeacher.$valid) {
            $scope.loading = true;
            $http({
                method: 'POST',
                url: '/account/createaccountteacher',
                data: $.param($scope.teacher),  //param method from jQuery
                headers: { 'Content-Type': 'application/x-www-form-urlencoded' }  //set the headers so angular passing info as form data (not request payload)
            }).success(function () {
                console.log(data);
                if (data.success) { //success comes from the return json object
...
homeSite.controller('CreateAccountStudentController', function ($scope, $http) {
    $scope.submit = function (teacherForm) {
